...
---
# `09-fences-advanced/` — the rod, IaC, engines, manifests
...
| `manifests/cargo/` | `~~~cargo as rs` | cargo | a script carrying its `Cargo.toml`; `check`, `run` |
...
| `manifests/cmake/` | `~~~cmake as cm` | cc | `configure`, `build`, `test`; sources named as `${BASHPP_CWD}/…` |
...
- **The directory is the caller's.** Each case `cd`s to its own directory
  ...
  `bashy awd 09-fences-advanced/manifests/cargo -- bashy --bashsharp build.bsh`.
